...
[{{< fa brands github >}} View source code](https://github.com/openpipelines-bio/openpipeline/tree/v4.2.0/src/qc/calculate_qc_metrics)

[![](/images/viash-hub-logo.svg){.vh-icon} Browse all versions](https://www.viash-hub.com/packages/openpipeline)

[![](/images/viash-hub-logo.svg){.vh-icon} Get run instructions](https://www.viash-hub.com/packages/openpipeline/v4.2.0/components/qc/calculate_qc_metrics)
...
Generate a `params.yaml` for this component on [Viash Hub](https://www.viash-hub.com/packages/openpipeline/v4.2.0/components/qc/calculate_qc_metrics).
...
|:----------|:--------------------------------------------------|:--------------------|
|`--input`           |Input data. Can be encoded in H5 or Zarr (v3) format.                                                                                                                                                                       |`file`, required, example: `"input.h5mu"` |
|`--modality`        |Which modality from the input MuData file to process.                                                                                                                                                                       |`string`, default: `"rna"`                |
|`--layer`           |Layer from modality to use as input data. If not provided the .X attribute is used.                                                                                                                                         |`string`, example: `"raw_counts"`         |
|`--log1p_transform` |Compute log1p-transformed versions of the metrics added to `.var` and `.obs`. For each emitted metric column `<col>`, an additional column `log1p_<col>` is added containing `log1p(<col>)`. Mirrors scanpy's `log1p` flag. |`boolean`, default: `TRUE`                |
...
|`--output_obs_top_n_vars`        |Format string for the names of the .obs columns that contain the cumulative proportion of counts in the top N vars (one column per value in `--top_n_vars`). Use `{n}` as a placeholder for the top-N value provided in `--top_n_vars`. |`string`, default: `"pct_of_counts_in_top_{n}_vars"`                                   |
...
|Name                   |Description                                                                                                    |Attributes                       |
|:----------|:--------------------------------------------------|:--------------------|
|`--output`             |Output data. Can be encoded using H5 or Zarr v3, matching the format of the input data.                        |`file`, example: `"output.h5mu"` |
