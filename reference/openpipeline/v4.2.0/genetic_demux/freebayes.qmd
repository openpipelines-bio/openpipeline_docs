...
[{{< fa brands github >}} View source code](https://github.com/openpipelines-bio/openpipeline/tree/v4.2.0/src/genetic_demux/freebayes)

[![](/images/viash-hub-logo.svg){.vh-icon} Browse all versions](https://www.viash-hub.com/packages/openpipeline)

[![](/images/viash-hub-logo.svg){.vh-icon} Get run instructions](https://www.viash-hub.com/packages/openpipeline/v4.2.0/components/genetic_demux/freebayes)
...
Generate a `params.yaml` for this component on [Viash Hub](https://www.viash-hub.com/packages/openpipeline/v4.2.0/components/genetic_demux/freebayes).
...
|:----------|:--------------------------------------------------|:--------------------|
|`--bam`                             |Add FILE to the set of BAM files to be analyzed.                                                                                                                                                                    |`file`, example: `"input.bam"`                   |
|`--bam_list`                        |A file containing a list of BAM files to be analyzed.                                                                                                                                                               |`file`, example: `"bam_list.txt"`                |
|`--stdin`                           |Read BAM input on stdin.                                                                                                                                                                                            |`boolean_true`                                   |
|`--fasta_reference`                 |Use FILE as the reference sequence for analysis. An index file (FILE.fai) will be created if none exists. If neither --targets nor --region are specified, FreeBayes will analyze every position in this reference. |`file`, example: `"reference.fasta"`             |
|`--fasta_reference_index`           |Use FILE.fai as the index of reference sequence for analysis.                                                                                                                                                       |`file`, example: `"file.fai"`                    |
|`--targets`                         |Limit analysis to targets listed in the BED-format FILE.                                                                                                                                                            |`file`, example: `"targets.bed"`                 |
|`--region`                          |Limit analysis to the specified region, 0-base coordinates, end_position not included (same as BED format).                                                                                                         |`string`                                         |
|`--samples`                         |Limit analysis to samples listed (one per line) in the FILE. By default FreeBayes will analyze all samples in its input BAM files.                                                                                  |`file`, example: `"samples.txt"`                 |
|`--populations`                     |Each line of FILE should list a sample and a population which it is part of. The population-based bayesian inference model will then be partitioned on the basis of the populations.                                |`file`, example: `"populations.txt"`             |
...
|`--variant_input`                   |Use variants reported in VCF file as input to the algorithm. Variants in this file will included in the output even if there is not enough support in the data to pass input filters.                               |`file`, example: `"variant_input.vcf"`           |
...
|`--observation_bias`                |Read length-dependent allele observation biases from FILE. The format is [length] [alignment efficiency relative to reference] where the efficiency is 1 if there is no relative observation bias.                  |`file`, example: `"observation_bias.txt"`        |
...
|`--contamination_estimates`         |A file containing per-sample estimates of contamination, such as those generated by VerifyBamID.                                                                                                                    |`file`, example: `"contamination_estimates.txt"` |
